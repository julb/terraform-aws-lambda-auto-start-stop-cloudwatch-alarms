...
# The lambda IAM role.
resource "aws_iam_role_policy" "this_cloudwatch_enable_alarms" {
  count = var.custom_iam_role_arn == null && contains(["start", "enable"], var.action) ? 1 : 0

  name = "${var.name}EnableAlarmsPolicy"
...
resource "aws_iam_role_policy" "this_cloudwatch_disable_alarms" {
  count = var.custom_iam_role_arn == null && contains(["stop", "disable"], var.action) ? 1 : 0

  name = "${var.name}DisableAlarmsPolicy"
  role = aws_iam_role.this[0].id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
