...
variable "lookup_resource_tag" {
  ...
  description = "The tag to use to search for CloudWatch alarms."
}

variable "lookup_resource_regions" {
  description = "A list of regions in which the resource will be looked for. By default, use the current region."
  type        = list(string)
  default     = null
}